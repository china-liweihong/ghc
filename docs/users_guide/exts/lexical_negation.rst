...
In Haskell 2010, the minus sign stands for negation when it is has no left-hand
...
This leads to certaian syntactic anomalies:
...
is enabled, ``-5`` is parsed as ``negate 5`` regardless of context, so ``f -5``
works as expected. However, it only applies to literals, so ``f -x`` or ``f
-(a*2)`` are still parsed as subtraction.
